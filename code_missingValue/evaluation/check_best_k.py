...
path_evaluation = r'E:\Semester 7\TA\code_missingValue\evaluation'
...
for file_path in csv_files:
    ...
    total_tp = df_metrics['Pantulan_Tepat'].sum()
    total_fp = df_metrics['Zona_Jebol'].sum()
    total_fn = df_metrics['Pantulan_Terlewat'].sum()
    total_tn = df_metrics['Abaikan_Valid'].sum()
...
print(f"{'Skenario':<10} | {'Emiten':<6} | {'TP':<6} | {'FP':<6} | {'FN':<6} | {'TN':<6} | {'Accuracy':<10} | {'Precision':<10} | {'Recall':<10} | {'F1-Score':<10}")
print("-"*110)
for _, row in df_summary.iterrows():
    print(f"{row['K_Value']:<10} | {row['Jumlah_Emiten']:<6} | {row['Total_TP']:<6} | {row['Total_FP']:<6} | {row['Total_FN']:<6} | {row['Total_TN']:<6} | {row['Avg_Accuracy']:10.4f} | {row['Avg_Precision']:10.4f} | {row['Avg_Recall']:10.4f} | {row['Avg_F1_Score']:10.4f}")
print("-"*110)
